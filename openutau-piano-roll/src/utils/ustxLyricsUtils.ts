...
export class USTXLyricsManager {
  ...
  private detectTimingGapPhrases(): VerseSegment[] {
    ...
    for (let i = 1; i < lyricalNotes.length; i++) {
      ...
      // Only consider SIGNIFICANT gaps as verse boundaries (not tiny gaps between words)
      // Use a threshold of 2400 ticks (equivalent to 1.5 beats at 480 resolution)
      const hasTimingGap = gap > 2400;
      
      // Debug key transitions and significant gaps
      if (gap > 2400 || prevNote.lyric === "grey" || prevNote.lyric === "away" || prevNote.lyric === "are") {
        console.log(`Timing gap analysis: "${prevNote.lyric}" → "${currentNote.lyric}"`, {
          ...
          gapSizeInBeats: (gap / 480).toFixed(1)
        });
      }
  ...
  async getLyricsMetadata(singerId: string = 'fem_1_ln') {
    console.log('Getting lyrics metadata using proper verse detection...');
    
    // Use the main detectVerses() method which prioritizes timing-based detection
    const verses = this.detectVerses();
    const allLyrics = this.getAllLyrics();
    
    // If main detection fails, fallback to simple detection
    const finalVerses = verses.length > 0 ? verses : this.detectVersesSimple();
    
    const metadata = {
      totalVerses: finalVerses.length,
      ...
      detectionMethod: verses.length > 0 ? 'timing-based' : 'simple-fallback'
    };

    console.log('Generated lyrics metadata with proper verse detection:', metadata);
    return metadata;
  }
}