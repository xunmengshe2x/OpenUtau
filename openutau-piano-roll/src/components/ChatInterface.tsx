...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiResponse = await processAIMessage(inputValue, ustxData);
      ...
      // Update USTX data if changes were made
      if (aiResponse.updatedUSTX && onUSTXUpdate) {
        onUSTXUpdate(aiResponse.updatedUSTX);
      }
  ...
  const processAIMessage = async (message: string, currentData?: USTXData) => {
    // This will be implemented to call OpenRouter API with DeepSeek R1
